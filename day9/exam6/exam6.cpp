...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		mywin32_engine::makeEditBox(hWnd, 0, 0, 250, 3001);			//경제성장률 입력
		mywin32_engine::makeMiniEditBox(hWnd, 0, 24, 3002);			//평균 경제성장률
		mywin32_engine::makeMiniEditBox(hWnd, 0, 24*2, 3003);		//평균 초과횟수 
	}
		break;
    case WM_COMMAND:
        {
			...
			case IDM_RUN:
			{
				...
				/*int a = 0;
				for (int i = 0; i < 10; i++) {
					if (growth[i] > lfSum / nCount)
						a++;
				}
				mywin32_engine::SetControlValueDouble(hWnd, 3003, a);*/
			}
				break;
            case IDM_ABOUT:
